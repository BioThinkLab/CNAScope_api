...
#SBATCH --cpus-per-task=1              # 每个任务使用的CPU核心数量
#SBATCH --mem=1G                       # 内存需求
...
# 检查参数数量
if [ $# -lt 4 ]; then
    echo "Error: Missing arguments. Usage: sbatch $0 <uuid> <input_files> <ref> <obs_type>"
    exit 1
fi

# 获取命令行参数
uuid=$1
input_files=$2  # 这可以是单个文件或逗号分隔的多个文件路径
ref=$3
obs_type=$4
...
# 构建数据集名称（使用UUID生成与输入文件数量相同的名称）
datasets=""
for ((i=0; i<${#INPUT_FILES_ARRAY[@]}; i++)); do
    if [ $i -eq 0 ]; then
        datasets="${uuid}"
    else
        datasets="${datasets},${uuid}_${i}"
    fi
done
...
# 执行recurrent_cna_anno.sh脚本
echo "Running recurrent_cna_anno.sh..."
bash "${script_dir}/run/recurrent_cna_anno.sh" \
    "${datasets}" "${input_files}" \
    "${output_dir}" "${ref}" "${obs_type}"
...
# 结束脚本，返回与recurrent_cna_anno.sh相同的退出状态码
